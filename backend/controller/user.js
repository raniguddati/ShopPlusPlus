...
export const signUpUser = async ({ name, email, password, address, phoneNumber }) => {
    try {
        ...
        return Promise.resolve({
            user: { id: user._id, name: user.name, lastLoggedIn: user.lastLoggedIn, address: user.address, phoneNumber: user.phoneNumber },
            token,
        });
    } catch (error) {
        return Promise.reject({ error });
